...
class ViewController(BaseController):
    ...
    def get_place_data(self):
        ...
        place = {
                ...
                "user_name": db_place.user.nickname,
                "user_avatar": db_place.user.avatar,
                "user_id": db_place.user.id,
                "latitude": float(db_place.latitude),
                "longitude": float(db_place.longitude),
                "num_comments": len(db_place.comments),
                "created_on": db_place.created_on.strftime('%d/%m/%Y %H:%M:%S'),
                "positive_scorings": len(db_place.positive_scorings),
                "visits": db_place.visits,
                "comment_image" : comment_image,
                "ph" : db_place.ph,
                "chlorine" : db_place.chlorine,
                "last_update": db_place.last_update.strftime('%d/%m/%Y %H:%M:%S'),
                "last_updater_name":db_place.comments[-1].user.nickname}
        ...
        return h.toJSON({
                ...
                'place': place,
                'comments': comments,
                'pages': int(math.ceil(float(len(db_place.comments)) / float(page_size))),
                'current_page': page})
